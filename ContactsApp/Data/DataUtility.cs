...
namespace ContactsApp.Data
{
    public static class DataUtility
    {
        ...
        public static async Task ManageDataAsync(IServiceProvider serviceProvider)
        {
            ...
            await SeedDemoContactsAsync(userManagerSvc, dbContextSvc, configurationSvc);
        }

        private static async Task SeedDemoUserAsync(UserManager<ApplicationUser> userManager, IConfiguration configuration)
        {
            try
            {
                string? demoEmail = configuration["DemoUserLogin"];
                string? demoPassword = configuration["DemoUserPassword"];

                if (string.IsNullOrEmpty(demoEmail) || string.IsNullOrEmpty(demoPassword)) 
                {
                    throw new Exception("Demo user Email and/or Password not found. Seeding Failed");
                }
            ...
            {
                Console.WriteLine("******************* ERROR  (DataUtility.cs) *******************");
